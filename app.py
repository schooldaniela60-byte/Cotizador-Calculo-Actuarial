...
with tab1:
    ...
    with col2:
        ...
        if tipo_seguro in ["Temporal", "Diferido temporal", "Dotal puro", "Dotal mixto"]:
        ...
        try:
            ...
            # ── Diferido temporal ─────────────────────────────────────────
                ...
                if x + m + n > 100:
                    ...
                    i   = idx(x);  im  = idx(x + m);  imn = idx(x + m + n);  ij = idx(x + n)
                    A   = (T["Mx"][im] - T["Mx"][imn]) / T["Dx"][i]
                    PNU = SA * A
                    ann = (T["Nx"][i] - T["Nx"][ij]) / T["Dx"][i]
                    PNN = PNU / ann
            ...
            # ── Dotal puro ────────────────────────────────────────────────
                ...
                if x + m + n > 100:
                    ...
                    i   = idx(x);  imn = idx(x + m + n);  ik = idx(x + k)
                    A   = T["Dx"][imn] / T["Dx"][i]
                    PNU = SA * A
                    ann = (T["Nx"][i] - T["Nx"][ik]) / T["Dx"][i]
                    PNN = PNU / ann

                    st.subheader(f"Cotización — {nombre if nombre else 'Asegurado'}")
                    st.caption(f"Dotal Puro · Edad actuarial: {x} · Temporalidad: {n} · Diferimiento: {m} · Pagos: {k}")
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Valores Conmutados",  f"{A:.6f}")
                    c2.metric("PNU",       f"${PNU:,.2f}")
                    c3.metric("PNN anual", f"${PNN:,.2f}")
            ...
            # ── Dotal mixto ───────────────────────────────────────────────
                ...
                if x + m + n > 100:
                    ...
                    i   = idx(x);  im  = idx(x + m);  imn = idx(x + m + n);  ik = idx(x + k)
                    ...
                    st.subheader(f"Cotización — {nombre if nombre else 'Asegurado'}")
                    st.caption(f"Dotal Mixto · Edad actuarial: {x} · Temporalidad: {n} · Diferimiento: {m} · Pagos: {k}")
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Valores Conmutados",  f"{A:.6f}")
                    c2.metric("PNU",       f"${PNU:,.2f}")
                    c3.metric("PNN anual", f"${PNN:,.2f}")
            ...
            # ── Vitalicio ─────────────────────────────────────────────────
                ...
                if x + m > 100:
                    ...
                    st.subheader(f"Cotización — {nombre if nombre else 'Asegurado'}")
                    st.caption(f"Vitalicio · Edad actuarial: {x} · Diferimiento: {m} · Pagos: {k}")
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Valores Conmutados",  f"{A:.6f}")
                    c2.metric("PNU",       f"${PNU:,.2f}")
                    c3.metric("PNN anual", f"${PNN:,.2f}")
